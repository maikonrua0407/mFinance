...
{
    public class DS_HT_TNGUYEN_LOAI
    {
        private string ENTITY_SET_NAME = "HT_TNGUYEN_LOAI";

        public bool Them(HT_TNGUYEN_LOAI obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_TNGUYEN_LOAI.AddObject(obj);
        ...
        public bool Sua(HT_TNGUYEN_LOAI obj)
        {
            bool kq = true;            
            Entities entities = ContextFactory.GetInstance();
            EntityKey key = null;
        ...
        public bool Xoa(HT_TNGUYEN_LOAI obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_TNGUYEN_LOAI obj = new HT_TNGUYEN_LOAI();
                        obj = entities.HT_TNGUYEN_LOAI.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_TNGUYEN_LOAI GetById(int id)
        {
            HT_TNGUYEN_LOAI kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_TNGUYEN_LOAI.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
        ...
        public HT_TNGUYEN_LOAI GetByMa(string ma)
        {
            HT_TNGUYEN_LOAI kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {                
                kq = entities.HT_TNGUYEN_LOAI.FirstOrDefault(e => e.MA_TNGUYEN_LOAI.Equals(ma));
                return kq;
            }
            catch (System.Exception ex)
            {
