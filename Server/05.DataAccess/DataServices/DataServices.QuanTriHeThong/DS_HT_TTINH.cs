...
{
    public class DS_HT_TTINH
    {
        private string ENTITY_SET_NAME = "HT_TTINH";

        public bool Them(HT_TTINH obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_TTINH.AddObject(obj);
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_TTINH obj = new HT_TTINH();
                        obj = entities.HT_TTINH.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_TTINH GetById(int id)
        {
            HT_TTINH kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_TTINH.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
        ...
        public HT_TTINH GetByMa(string ma)
        {
            HT_TTINH kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {                
                kq = entities.HT_TTINH.FirstOrDefault(e => e.MA_TTINH.Equals(ma));
                return kq;
            }
            catch (System.Exception ex)
            {
