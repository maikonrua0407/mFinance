...
{
    public class DS_HT_TVAN 
    {
        private string ENTITY_SET_NAME = "HT_TVAN";

        public bool Them(HT_TVAN  obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_TVAN .AddObject(obj);
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_TVAN  obj = new HT_TVAN ();
                        obj = entities.HT_TVAN .FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_TVAN  GetById(int id)
        {
            HT_TVAN  kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_TVAN .FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
        ...
        public HT_TVAN  GetByMa(string ma)
        {
            HT_TVAN  kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {                
                kq = entities.HT_TVAN .FirstOrDefault(e => e.MA_TVAN.Equals(ma));
                return kq;
            }
            catch (System.Exception ex)
            {
