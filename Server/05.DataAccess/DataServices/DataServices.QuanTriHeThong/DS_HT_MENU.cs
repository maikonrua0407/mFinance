...
{
    public class DS_HT_MENU
    {
        private string ENTITY_SET_NAME = "HT_MENU";

        public bool Them(HT_MENU obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_MENU.AddObject(obj);
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    foreach (int id in listID)
                    {
                        ...
                        obj = entities.HT_MENU.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_MENU GetById(int id)
        {
            HT_MENU kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                kq = entities.HT_MENU.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
        ...
        public HT_MENU GetByMa(string ma)
        {
            HT_MENU kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                kq = entities.HT_MENU.FirstOrDefault(e => e.MA_CNANG.Equals(ma));
                return kq;
            }
            catch (System.Exception ex)
            {
        ...
        public List<HT_MENU> GetListByIdChucNang(List<int> lstIdChucNang)
        {
            List<HT_MENU> kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                kq = entities.HT_MENU.Where(e => lstIdChucNang.Contains(e.ID_CNANG.Value)).ToList();
                return kq;
            }
            catch (System.Exception ex)
            {
