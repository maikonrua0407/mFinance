...
{
    public class DS_HT_MENU_HDSD
    {
        private string ENTITY_SET_NAME = "HT_MENU_HDSD";

        public bool Them(HT_MENU_HDSD obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_MENU_HDSD.AddObject(obj);
        ...
        public bool Sua(HT_MENU_HDSD obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            EntityKey key = null;
        ...
        public bool Xoa(HT_MENU_HDSD obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_MENU_HDSD obj = new HT_MENU_HDSD();
                        obj = entities.HT_MENU_HDSD.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_MENU_HDSD GetById(int id)
        {
            HT_MENU_HDSD kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                kq = entities.HT_MENU_HDSD.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
        ...
        public List<HT_MENU_HDSD> GetByMaMenu(string maMenu)
        {
            List<HT_MENU_HDSD> kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                kq = entities.HT_MENU_HDSD.Where(e => e.MA_MENU.Equals(maMenu)).ToList();
                return kq;
            }
            catch (System.Exception ex)
            {
