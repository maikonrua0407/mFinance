...
{
    public class DS_HT_CNANG_TSO
    {
        private string ENTITY_SET_NAME = "HT_CNANG_TSO";

        public bool Them(HT_CNANG_TSO obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_CNANG_TSO.AddObject(obj);
        ...
        public bool Sua(HT_CNANG_TSO obj)
        {
            bool kq = true;            
            Entities entities = ContextFactory.GetInstance();
            EntityKey key = null;
        ...
        public bool Xoa(HT_CNANG_TSO obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_CNANG_TSO obj = new HT_CNANG_TSO();
                        obj = entities.HT_CNANG_TSO.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_CNANG_TSO GetById(int id)
        {
            HT_CNANG_TSO kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_CNANG_TSO.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
