...
{
    public class DS_HT_TNGUYEN_KBAO
    {
        private string ENTITY_SET_NAME = "HT_TNGUYEN_KBAO";

        public bool Them(HT_TNGUYEN_KBAO obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_TNGUYEN_KBAO.AddObject(obj);
        ...
        public bool Sua(HT_TNGUYEN_KBAO obj)
        {
            bool kq = true;            
            Entities entities = ContextFactory.GetInstance();
            EntityKey key = null;
        ...
        public bool Xoa(HT_TNGUYEN_KBAO obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_TNGUYEN_KBAO obj = new HT_TNGUYEN_KBAO();
                        obj = entities.HT_TNGUYEN_KBAO.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_TNGUYEN_KBAO GetById(int id)
        {
            HT_TNGUYEN_KBAO kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_TNGUYEN_KBAO.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
