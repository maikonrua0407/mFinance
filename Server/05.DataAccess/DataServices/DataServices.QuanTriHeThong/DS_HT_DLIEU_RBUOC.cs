...
{
    public class DS_HT_DLIEU_RBUOC
    {
        private string ENTITY_SET_NAME = "HT_DLIEU_RBUOC";

        public bool Them(HT_DLIEU_RBUOC obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_DLIEU_RBUOC.AddObject(obj);
        ...
        public bool Sua(HT_DLIEU_RBUOC obj)
        {
            bool kq = true;            
            Entities entities = ContextFactory.GetInstance();
            EntityKey key = null;
        ...
        public bool Xoa(HT_DLIEU_RBUOC obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_DLIEU_RBUOC obj = new HT_DLIEU_RBUOC();
                        obj = entities.HT_DLIEU_RBUOC.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_DLIEU_RBUOC GetById(int id)
        {
            HT_DLIEU_RBUOC kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_DLIEU_RBUOC.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
