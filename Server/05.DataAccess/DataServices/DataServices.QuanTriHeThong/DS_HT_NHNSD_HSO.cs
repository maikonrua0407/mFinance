...
{
    public class DS_HT_NHNSD_HSO
    {
        private string ENTITY_SET_NAME = "HT_NHNSD_HSO";

        public bool Them(HT_NHNSD_HSO obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_NHNSD_HSO.AddObject(obj);
        ...
        public bool Sua(HT_NHNSD_HSO obj)
        {
            bool kq = true;            
            Entities entities = ContextFactory.GetInstance();
            EntityKey key = null;
        ...
        public bool Xoa(HT_NHNSD_HSO obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_NHNSD_HSO.DeleteObject(obj);
                entities.SaveChanges();
            }
            catch (System.Exception ex)
            {
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_NHNSD_HSO obj = new HT_NHNSD_HSO();
                        obj = entities.HT_NHNSD_HSO.FirstOrDefault(e => e.ID == id);
        ...
        public HT_NHNSD_HSO GetById(int id)
        {
            HT_NHNSD_HSO kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_NHNSD_HSO.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
