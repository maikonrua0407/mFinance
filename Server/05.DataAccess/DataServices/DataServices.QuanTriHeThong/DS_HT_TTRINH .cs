...
{
    public class DS_HT_TTRINH 
    {
        private string ENTITY_SET_NAME = "HT_TTRINH";

        public bool Them(HT_TTRINH  obj)
        {
            bool kq = true;
            Entities entities = ContextFactory.GetInstance();
            try
            {
                entities.HT_TTRINH.AddObject(obj);
        ...
        public bool XoaListTheoID(List<int> listID)
        {
            ...
            try
            {
                using (TransactionScope trans = new TransactionScope())
                {
                    ...
                    {
                        HT_TTRINH  obj = new HT_TTRINH ();
                        obj = entities.HT_TTRINH.FirstOrDefault(e => e.ID == id);
                        entities.DeleteObject(obj);
                        entities.SaveChanges();
                    }
                    trans.Complete();
        ...
        public HT_TTRINH  GetById(int id)
        {
            HT_TTRINH  kq = null;
            Entities entities = ContextFactory.GetInstance();                
            try
            {
                kq = entities.HT_TTRINH.FirstOrDefault(e => e.ID == id);
                return kq;
            }
            catch (System.Exception ex)
            {
        ...
        public HT_TTRINH  GetByMa(string ma)
        {
            HT_TTRINH  kq = null;
            Entities entities = ContextFactory.GetInstance();
            try
            {                
                kq = entities.HT_TTRINH.FirstOrDefault(e => e.MA_TTRINH.Equals(ma));
                return kq;
            }
            catch (System.Exception ex)
            {
