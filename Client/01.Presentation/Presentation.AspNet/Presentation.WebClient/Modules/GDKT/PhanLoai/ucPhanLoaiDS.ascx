<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ucPhanLoaiDS.ascx.cs" Inherits="Presentation.WebClient.Modules.GDKT.PhanLoai.ucPhanLoaiDS" %>
...
<script language="javascript" type="text/javascript">
    var griddataid = '<%=grdPhanLoaiDS.ClientID %>'
    var oTable;
        ...
        $('#' + griddataid).on('dblclick', 'tr', function () {
            if (this.rowIndex > 0) {
                ...
                for (j = 0; j < tcb.length; j++) {
                    ...
                    IDPLOAI = tcb[j].id.replace('cbrow', '')
                    break
                }

                if (IDPLOAI != 0) {
                    showpopup(linkpopup + "&ID=" + IDPLOAI + "&v=1", window.outerWidth - 120, window.outerHeight - 100)
                }
            }
        ...
        $("#btnEdit").on("click", function () {
            ...
            for (j = 0; j < scb.length; j++) {
                if (getObj(scb[j].id).checked == true) {
                    IDPLOAI = scb[j].id.replace('cbrow', '')
                    break;
                }
            }

            if (IDPLOAI != 0) {
                showpopup(linkpopup + "&ID=" + IDPLOAI + "&e=1", window.outerWidth - 120, window.outerHeight - 100)
            }
        });
                ...
                <td valign="top">
                    ...
                    <asp:Panel ID="pnDanhsach" runat="server" GroupingText="Danh sách" CssClass="TitlePanel">
                        ...
                        <div class="scroll-pane" id="divtableContent" style="min-height: 200px">
                            <asp:DataGrid runat="server" ID="grdPhanLoaiDS" AutoGenerateColumns="false"
                                ...
                                <Columns>
                                    ...
                                    <asp:TemplateColumn>
                                        ...
                                        <HeaderTemplate>
                                            <%=LanguageEngine.Instance().GetContent(LanguageType.TypeUI, "U.KeToan.PhanLoai.ucPhanLoaiDS.MaPhanLoaiTK")%>
                                        </HeaderTemplate>
                                        <ItemTemplate>
                                            <%#DataBinder.Eval(Container.DataItem, "MA_PLOAI")%>
                                        </ItemTemplate>
                                    </asp:TemplateColumn>
                                    <asp:TemplateColumn>
                                        <HeaderStyle HorizontalAlign="Center"></HeaderStyle>
                                        <ItemStyle HorizontalAlign="left"></ItemStyle>
                                        <HeaderTemplate>
                                            <%=LanguageEngine.Instance().GetContent(LanguageType.TypeUI, "U.KeToan.PhanLoai.ucPhanLoaiDS.TenPhanLoaiTK")%>
                                        </HeaderTemplate>
                                        <ItemTemplate>
                                            <%#DataBinder.Eval(Container.DataItem, "TEN_PLOAI")%>
                                        </ItemTemplate>
                                    </asp:TemplateColumn>
                                    <asp:TemplateColumn>
                                        <HeaderStyle HorizontalAlign="Center"></HeaderStyle>
                                        <ItemStyle HorizontalAlign="left"></ItemStyle>
                                        <HeaderTemplate>
                                            <%=LanguageEngine.Instance().GetContent(LanguageType.TypeUI, "U.KeToan.PhanLoai.ucPhanLoaiDS.TinhChat")%>
                                        </HeaderTemplate>
                                        <ItemTemplate>
                                            <%#DataBinder.Eval(Container.DataItem, "TEN_NHOM_PLOAI")%>
                                        </ItemTemplate>
                                    </asp:TemplateColumn>
                                    <asp:TemplateColumn>
                                        <HeaderStyle HorizontalAlign="Center"></HeaderStyle>
                                        <ItemStyle HorizontalAlign="left"></ItemStyle>
                                        <HeaderTemplate>
                                            <%=LanguageEngine.Instance().GetContent(LanguageType.TypeUI, "U.KeToan.PhanLoai.ucPhanLoaiDS.MaPhanLoaiTKCha")%>
                                        </HeaderTemplate>
                                        <ItemTemplate>
                                            <%#DataBinder.Eval(Container.DataItem, "MA_PLOAI_CHA")%>
                                        </ItemTemplate>
                                    </asp:TemplateColumn>
                                    <asp:TemplateColumn>
                                        <HeaderStyle HorizontalAlign="Center"></HeaderStyle>
                                        <ItemStyle HorizontalAlign="Left"></ItemStyle>
                                        <HeaderTemplate>
                                            <%=LanguageEngine.Instance().GetContent(LanguageType.TypeUI, "U.KeToan.PhanLoai.ucPhanLoaiDS.TheoDoiCongNo")%>
                                        </HeaderTemplate>
                                        <ItemTemplate>
                                            <%#DataBinder.Eval(Container.DataItem, "TCHAT_CNO")%>
                                        </ItemTemplate>
                                    </asp:TemplateColumn>
                                    <asp:TemplateColumn>
                                        <HeaderStyle HorizontalAlign="Center"></HeaderStyle>
                                        <ItemStyle HorizontalAlign="Left"></ItemStyle>
                                        <HeaderTemplate>
                                            <%=LanguageEngine.Instance().GetContent(LanguageType.TypeUI, "U.KeToan.PhanLoai.ucPhanLoaiDS.NgayApDung")%>
                                        </HeaderTemplate>
                                        <ItemTemplate>
                                    ...
                                    <asp:TemplateColumn>
                                        ...
                                        <ItemStyle HorizontalAlign="Left"></ItemStyle>
                                        <HeaderTemplate>
                                            <%=LanguageEngine.Instance().GetContent(LanguageType.TypeUI, "U.KeToan.PhanLoai.ucPhanLoaiDS.TrangThai")%>
                                        </HeaderTemplate>
                                        <ItemTemplate>
