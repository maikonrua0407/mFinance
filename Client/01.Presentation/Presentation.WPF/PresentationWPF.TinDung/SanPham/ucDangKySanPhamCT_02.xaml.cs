...
{
    /// <summary>
    /// Interaction logic for ucDangKySanPhamCT_02.xaml
    /// </summary>
    public partial class ucDangKySanPhamCT_02 : UserControl
    {
        /// <summary>
        /// Khai bao
        /// </summary>
        ...
        /// </summary>
        #region Khoi tao
        public ucDangKySanPhamCT_02()
        {
            InitializeComponent();
            HeThong htHeThong = new HeThong();
            htHeThong.DuyetQuyenTinhNangToolbar("/PresentationWPF.TinDung;component/SanPham/ucDangKySanPhamCT_02.xaml", ref Toolbar, ref mnuMain);
            ...
            thamSoBuTru = new UtilitiesProcess().LayGiaTriThamSo(BusinessConstant.LoaiThamSo.TW, BusinessConstant.MaThamSo.TW_DIEM_BUTRU_GOCLAI, ClientInformation.MaDonVi);
            if (thamSoBuTru.IsNullOrEmptyOrSpace())
                thamSoBuTru = "CKY";
        ...
        private void ShowControl()
        {
            ...
            arr = hethong.SetVisibleControl("PresentationWPF.TinDung.SanPham.ucDangKySanPhamCT_02", "");
            ...
            grbLaiSuat.Header = LLanguage.SearchResourceByKey("U.TinDung.ucDangKySanPhamCT_02.GrLaiSuat", new string[1] { LNumber.ReadNumber(sBaremTinhLai.StringToDecimal()).ToLower() });
        }
        ...
        private void LockControlGridView()
        {
            ...
            {
                raddgrGocLaiVayDS.Columns[7].IsReadOnly = true;
                raddgrGocLaiVayDS.Columns[9].IsReadOnly = false;
                raddgrGocLaiVayDS.Columns[12].IsReadOnly = false;
            }
            else if (maPhuongThucTLai.Equals("DNO_GDAN") && maLoaiSanPham.Equals("VON_THOI_VU"))
            {
                raddgrGocLaiVayDS.Columns[9].IsReadOnly = true;
                raddgrGocLaiVayDS.Columns[7].IsReadOnly = false;
        ...
        private void LoadDuLieuThongTinChung(DataSet dsThongTin)
        {
            ...
            obj.SAN_PHAM_TD.CAN_CU_TINH_LAI_QH = dsThongTin.Tables[0].Rows[0]["CAN_CU_TINH_LAI_QH"].ToString();
            obj.SAN_PHAM_TD.TY_LE_TINH_LAI_QH = Convert.ToDecimal(dsThongTin.Tables[0].Rows[0]["TY_LE_TINH_LAI_QH"]);
            cmbCanCuTinhLaiQuaHan.SelectedIndex = lstCanCuTinhLaiQH.IndexOf(lstCanCuTinhLaiQH.FirstOrDefault(i => i.KeywordStrings.FirstOrDefault().Equals(obj.SAN_PHAM_TD.CAN_CU_TINH_LAI_QH)));
            numTyLeLaiQuaHan.Value = Convert.ToDouble(obj.SAN_PHAM_TD.TY_LE_TINH_LAI_QH);
            LayThongTinLSuat();
        }
        ...
        private void LoadDuLieuTaiKhoanHachToan(string sDoiTuong)
        {
            ...
            dv.RowFilter = "MA_KY_HIEU NOT IN ('CVN2','CVN3','CVN4','CVN5')";
            grdTKhoan.ItemsSource = dv;
        }
        ...
        void raddgrGocLaiVayDS_CellEditEnded(object sender, GridViewCellEditEndedEventArgs e)
        {
            ...
            {
                if (maPhuongThucTLai.Equals("DNO_BDAU"))
                {
            ...
            if (e.Cell.Column.UniqueName == "SO_TIEN_GOC")
            {
                ...
                else if (maPhuongThucTLai.Equals("DNO_GDAN"))
                {
                    decimal LaiSuat = 0;
                    TinhToanLaiSuat(out LaiSuat);
                    decimal SoTienMoiKy;
                    ...
                    decimal TongGocLaiMoiKy;
                    SoTienMoiKy = Convert.ToDecimal(e.NewData);
                    TinhToanSoTienGocLaiTheoDuNoGiamDan(LaiSuat, SoTienMoiKy, objVongVay.KY_HAN, out TongGocLaiMoiKy, out SoTienGocMoiKy, out SoTienLaiMoiKy, out TongGoc, out TongLai, out TongGocLai);
                    objVongVay.SO_TIEN_GOC_MOI_KY = SoTienGocMoiKy;
                    objVongVay.SO_TIEN_LAI_MOI_KY = SoTienLaiMoiKy;
                    objVongVay.SO_TIEN_GOC = TongGoc;
                    objVongVay.SO_TIEN_LAI = TongLai;
        ...
        void LayDuLieu(ref List<KT_PHAN_HE_PLOAI> lstPhanHe, BusinessConstant.TrangThaiBanGhi bangghi, BusinessConstant.TrangThaiNghiepVu nghiepvu)
        {
            ...
            obj.SAN_PHAM_TD.CAN_CU_TINH_LAI_QH = sCanCuTinhLQH;
            obj.SAN_PHAM_TD.TY_LE_TINH_LAI_QH = Convert.ToDecimal(numTyLeLaiQuaHan.Value.GetValueOrDefault());
            ...
            {
                if (drv["MA_KY_HIEU"].ToString().Equals("CVN1"))
                {
                    tkhoanvay = drv["MA_PLOAI"].ToString();
                    tkhoanvayBSo = drv["MA_PLOAI_BSO"].ToString();
                }
                ...
                if (!drv["MA_KY_HIEU"].ToString().Contains("CVN"))
                {
                ...
                {
                    var stringBuilder = new StringBuilder(tkhoanvay);
                    stringBuilder.Remove(3,1);
                    stringBuilder.Insert(3,drv["MA_KY_HIEU"].ToString().Substring(3,1));
                    objPhanHePLoai.MA_PLOAI = stringBuilder.ToString();
                    var stringBuilderBSo = new StringBuilder(tkhoanvayBSo);
                    stringBuilderBSo.Remove(3, 1);
                    stringBuilderBSo.Insert(3, drv["MA_KY_HIEU"].ToString().Substring(3, 1));
                    objPhanHePLoai.MA_PLOAI_BSO = stringBuilderBSo.ToString();
                }
        ...
        private void TinhToanSoTienGocLaiTheoDuNoGiamDan(decimal laiSuat, decimal soTien, int kyHan, out decimal tongGocLaiMoiKy, out decimal gocMoiKy, out decimal laiMoiKy, out decimal tongGoc, out decimal tongLai, out decimal tongGocLai)
        {
            DataTable dt = new DataTable();
            dt.Columns.Add("DU_GOC", typeof(decimal));
            dt.Columns.Add("LAI_SUAT", typeof(decimal));
            dt.Columns.Add("LAI", typeof(decimal));
            decimal soTienGocMoiKy = 0;
            decimal gocChenhLech = 0;
            decimal duGoc = 0;
            if (LObject.IsNullOrEmpty(Round) || !Round.IsNumeric())
                Round = "1";
            ...
            tongLai = 0;
            tongGocLai = 0;
            tongGoc = duGoc = soTien;

            gocMoiKy = soTienGocMoiKy = ((tongGoc / kyHan) / Round.StringToInt32()).Rounding(0) * Round.StringToInt32();
            gocChenhLech = tongGoc - soTienGocMoiKy * (kyHan - 1);
            for (int i = 0; i < kyHan; i++)
            {
                DataRow dr = dt.NewRow();
                dr["DU_GOC"] = duGoc;
                dr["LAI_SUAT"] = laiSuat;
                dr["LAI"] = ((duGoc * laiSuat / 100) / Round.StringToInt32()).Rounding(0) * Round.StringToInt32();
                dt.Rows.Add(dr);
                if (thamSoBuTru.Equals("DKY") && i == 0)
                    duGoc = duGoc - gocChenhLech;
                else if (thamSoBuTru.Equals("CKY") && i + 2 == kyHan)
                    duGoc = duGoc - gocChenhLech;
                else
                    duGoc = duGoc - soTienGocMoiKy;
            }
            tongLai = dt.AsEnumerable().Sum(f => f.Field<decimal>("LAI"));
            laiMoiKy = ((tongLai / kyHan)/Round.StringToInt32()).Rounding(0) * Round.StringToInt32();
            tongGocLai = tongGoc + tongLai;
            tongGocLaiMoiKy = gocMoiKy + laiMoiKy;
        }
